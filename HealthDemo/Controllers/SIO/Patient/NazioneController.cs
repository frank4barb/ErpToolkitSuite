...
namespace HealthDemo.Controllers.SIO.Patient
{
    public class NazioneController : ControllerErp
    {
        private static readonly List<string> xrefTables = new List<string> {
             "PaIdNazioneNascita"   //carico in cache tutti i dati di Paziente collegati
            ,"PaIdCittadinanza"   //carico in cache tutti i dati di Paziente collegati
            ,"PaIdNazioneDom"   //carico in cache tutti i dati di Paziente collegati
            ,"PaIdNazioneRes"   //carico in cache tutti i dati di Paziente collegati
        };
        private const string ErpContext_dogCache = "@HealthDemo.Controllers.SIO.Patient.Nazione_dogCache";
        ...
        public JsonResult AutocompleteGetAll(string? modelPropertyName = null, [FromQuery] List<string> extraFieldNames = null)
        {
            try
            {
                ...
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompleteGetAll<Nazione>(modelPropertyName: modelPropertyName, extraFieldNames: extraFieldNames));
            }
        ...
        public JsonResult AutocompleteGetSelect(string term, string? modelPropertyName = null, [FromQuery] Dictionary<string, List<string>> extraFields = null)
        {
            try
            {
                ...
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompleteGetSelect<Nazione>(term, modelPropertyName: modelPropertyName, extraFields: extraFields));
            }
        ...
        public JsonResult AutocompletePreLoad([FromBody] List<string> values)
        {
            try
            {
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompletePreLoad<Nazione>(values));
            }
       ...
       // -- GET: lista tipi documento ----------------------------
        ...
        public IActionResult ViewXdata(string icode) 
        {
            try
            {
                ...
                BlobStreamResult blob = ErpContext.Instance.DogFactory.GetDog(dogId).OpenBlobStream<Nazione>(icode, 0);
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: ViewXdata Nazione: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> AddXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            try
            {
                ...
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<Nazione>('A', null, null, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: AddXdataModel Nazione: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> UpdateXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            ...
            {
                if (file == null || file.Length == 0) return BadRequest("File mancante");
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<Nazione>('M', icode, timestampHex, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: UpdateXdata Nazione: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> DeleteXdata(string icode, string timestampHex)
        {
            try
            {
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<Nazione>('D', icode, timestampHex, null, "", "", null, null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: DeleteXdata Nazione: " + ex.Message }); }
        }
        [BindProperty]
        ...
        public ActionResult Index(SelNazione selobj)
        {
            ...
            //carica lista
            try { this.List = ErpContext.Instance.DogFactory.GetDog(dogId).List<Nazione>(this.Select, xrefTables, false, null, ref this._dogCache, null, -1); }  
        ...
        public IActionResult ReadForEdit([FromBody] ModelParam parms)  
        {
            ...
            Nazione obj = this.ReadForEditModel<Nazione>(parms, xrefTables, ref this._dogCache, prefix: modelPrefix);
        ...
        public IActionResult Save([FromBody] ModelObject dataObj)
        {
            string modelPrefix = "EDIT";
            ViewData.TemplateInfo.HtmlFieldPrefix = modelPrefix;  //prefisso da applicare a id e name nei tag, se uso lo stesso @model più volte nella stessa pagina eg: <xx id="EDIT_IdPatient" name="EDIT.IdPatient" ..>
            Nazione obj = this.SaveModel<Nazione>(dataObj, ref this._dogCache, prefix: modelPrefix, options: "*allowTouch*");
        ...
        public IActionResult ReadForDelete([FromBody] ModelParam parms)  
        {
            ...
            ViewData.TemplateInfo.HtmlFieldPrefix = modelPrefix;  //prefisso da applicare a id e name nei tag, se uso lo stesso @model più volte nella stessa pagina eg: <xx id="EDIT_IdPatient" name="EDIT.IdPatient" ..>
            Nazione obj = this.ReadForEditModel<Nazione>(parms, null, ref this._dogCache, prefix: modelPrefix, action: 'D');    // non carico tabelle relazionate per il delete
        ...
        public IActionResult Delete([FromBody] ModelObject dataObj)
        {
            string modelPrefix = "DELETE";
            ViewData.TemplateInfo.HtmlFieldPrefix = modelPrefix;  //prefisso da applicare a id e name nei tag, se uso lo stesso @model più volte nella stessa pagina eg: <xx id="EDIT_IdPatient" name="EDIT.IdPatient" ..>
            Nazione obj = this.SaveModel<Nazione>(dataObj, ref this._dogCache, prefix: modelPrefix, options: "[MAX_ONE_OBJ] [NO_ADD] [NO_UPDATE]");
            if (!ModelState.IsValid) { return this.ValidationResult(); }
