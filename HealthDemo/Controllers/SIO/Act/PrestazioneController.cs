    ...
    public class PrestazioneController : ControllerErp
    {
        ...
        public JsonResult AutocompleteGetSelect(string term, string? modelPropertyName = null, [FromQuery] Dictionary<string, List<string>> extraFields = null)
        {
            try
            {
                ...
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompleteGetSelect<Prestazione>(term, modelPropertyName: modelPropertyName, extraFields: extraFields));
            }
       ...
       // -- GET: lista tipi documento ----------------------------
        ...
        public IActionResult ViewXdata(string icode) 
        {
            try
            {
                ...
                BlobStreamResult blob = ErpContext.Instance.DogFactory.GetDog(dogId).OpenBlobStream<Prestazione>(icode, 0);
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: ViewXdata Prestazione: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> AddXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            try
            {
                ...
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<Prestazione>('A', null, null, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: AddXdataModel Prestazione: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> UpdateXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            ...
            {
                if (file == null || file.Length == 0) return BadRequest("File mancante");
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<Prestazione>('M', icode, timestampHex, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: UpdateXdata Prestazione: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> DeleteXdata(string icode, string timestampHex)
        {
            try
            {
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<Prestazione>('D', icode, timestampHex, null, "", "", null, null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: DeleteXdata Prestazione: " + ex.Message }); }
        }
        [BindProperty]
        ...
        public ActionResult Index(SelPrestazione selobj)
        {
            ...
            //carica lista
            try { this.List = ErpContext.Instance.DogFactory.GetDog(dogId).List<Prestazione>(this.Select, xrefTables, false, null, ref this._dogCache, null, -1); }  
        ...
        public IActionResult ReadForEdit([FromBody] ModelParam parms)  
        {
            ...
            Prestazione obj = this.ReadForEditModel<Prestazione>(parms, xrefTables, ref this._dogCache, prefix: modelPrefix);
        ...
        public IActionResult Save([FromBody] ModelObject dataObj)
        {
            string modelPrefix = "EDIT";
            ViewData.TemplateInfo.HtmlFieldPrefix = modelPrefix;  //prefisso da applicare a id e name nei tag, se uso lo stesso @model più volte nella stessa pagina eg: <xx id="EDIT_IdPatient" name="EDIT.IdPatient" ..>
            Prestazione obj = this.SaveModel<Prestazione>(dataObj, ref this._dogCache, prefix: modelPrefix, options: "*allowTouch*");
