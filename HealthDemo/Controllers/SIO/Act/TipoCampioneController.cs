...
namespace HealthDemo.Controllers.SIO.Act
{
    public class TipoCampioneController : ControllerErp
    {
        private static readonly List<string> xrefTables = new List<string> {
             "PcIdTipoCampione"   //carico in cache tutti i dati di RelPrestazioneCampione collegati
            ,"AcIdTipoCampione"   //carico in cache tutti i dati di RelAttivitaTipoCampione collegati
            ,"CpIdTipoCampione"   //carico in cache tutti i dati di Campione collegati
        };
        private const string ErpContext_dogCache = "@HealthDemo.Controllers.SIO.Act.TipoCampione_dogCache";
        ...
        public JsonResult AutocompleteGetAll(string? modelPropertyName = null, [FromQuery] List<string> extraFieldNames = null)
        {
            try
            {
                ...
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompleteGetAll<TipoCampione>(modelPropertyName: modelPropertyName, extraFieldNames: extraFieldNames));
            }
        ...
        public JsonResult AutocompleteGetSelect(string term, string? modelPropertyName = null, [FromQuery] Dictionary<string, List<string>> extraFields = null)
        {
            try
            {
                ...
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompleteGetSelect<TipoCampione>(term, modelPropertyName: modelPropertyName, extraFields: extraFields));
            }
        ...
        public JsonResult AutocompletePreLoad([FromBody] List<string> values)
        {
            try
            {
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompletePreLoad<TipoCampione>(values));
            }
       ...
       // -- GET: lista tipi documento ----------------------------
        ...
        public IActionResult ViewXdata(string icode) 
        {
            try
            {
                ...
                BlobStreamResult blob = ErpContext.Instance.DogFactory.GetDog(dogId).OpenBlobStream<TipoCampione>(icode, 0);
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: ViewXdata TipoCampione: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> AddXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            try
            {
                ...
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<TipoCampione>('A', null, null, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: AddXdataModel TipoCampione: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> UpdateXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            ...
            {
                if (file == null || file.Length == 0) return BadRequest("File mancante");
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<TipoCampione>('M', icode, timestampHex, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: UpdateXdata TipoCampione: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> DeleteXdata(string icode, string timestampHex)
        {
            try
            {
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<TipoCampione>('D', icode, timestampHex, null, "", "", null, null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: DeleteXdata TipoCampione: " + ex.Message }); }
        }
        [BindProperty]
        ...
        public ActionResult Index(SelTipoCampione selobj)
        {
            ...
            //carica lista
            try { this.List = ErpContext.Instance.DogFactory.GetDog(dogId).List<TipoCampione>(this.Select, xrefTables, false, null, ref this._dogCache, null, -1); }  
        ...
        public IActionResult ReadForEdit([FromBody] ModelParam parms)  
        {
            ...
            TipoCampione obj = this.ReadForEditModel<TipoCampione>(parms, xrefTables, ref this._dogCache, prefix: modelPrefix);
        ...
        public IActionResult Save([FromBody] ModelObject dataObj)
        {
            string modelPrefix = "EDIT";
            ViewData.TemplateInfo.HtmlFieldPrefix = modelPrefix;  //prefisso da applicare a id e name nei tag, se uso lo stesso @model più volte nella stessa pagina eg: <xx id="EDIT_IdPatient" name="EDIT.IdPatient" ..>
            TipoCampione obj = this.SaveModel<TipoCampione>(dataObj, ref this._dogCache, prefix: modelPrefix, options: "*allowTouch*");
        ...
        public IActionResult ReadForDelete([FromBody] ModelParam parms)  
        {
            ...
            ViewData.TemplateInfo.HtmlFieldPrefix = modelPrefix;  //prefisso da applicare a id e name nei tag, se uso lo stesso @model più volte nella stessa pagina eg: <xx id="EDIT_IdPatient" name="EDIT.IdPatient" ..>
            TipoCampione obj = this.ReadForEditModel<TipoCampione>(parms, null, ref this._dogCache, prefix: modelPrefix, action: 'D');    // non carico tabelle relazionate per il delete
        ...
        public IActionResult Delete([FromBody] ModelObject dataObj)
        {
            string modelPrefix = "DELETE";
            ViewData.TemplateInfo.HtmlFieldPrefix = modelPrefix;  //prefisso da applicare a id e name nei tag, se uso lo stesso @model più volte nella stessa pagina eg: <xx id="EDIT_IdPatient" name="EDIT.IdPatient" ..>
            TipoCampione obj = this.SaveModel<TipoCampione>(dataObj, ref this._dogCache, prefix: modelPrefix, options: "[MAX_ONE_OBJ] [NO_ADD] [NO_UPDATE]");
            if (!ModelState.IsValid) { return this.ValidationResult(); }
