    ...
    public class AttivitaController : ControllerErp
    {
        ...
        public JsonResult AutocompleteGetAll(string? modelPropertyName = null, [FromQuery] List<string> extraFieldNames = null)
        {
            try
            {
                ...
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompleteGetAll<Attivita>(modelPropertyName: modelPropertyName, extraFieldNames: extraFieldNames));
            }
        ...
        public JsonResult AutocompleteGetSelect(string term, string? modelPropertyName = null, [FromQuery] Dictionary<string, List<string>> extraFields = null)
        {
            try
            {
                ...
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompleteGetSelect<Attivita>(term, modelPropertyName: modelPropertyName, extraFields: extraFields));
            }
       ...
       // -- GET: lista tipi documento ----------------------------
        ...
        public IActionResult ViewXdata(string icode) 
        {
            try
            {
                ...
                BlobStreamResult blob = ErpContext.Instance.DogFactory.GetDog(dogId).OpenBlobStream<Attivita>(icode, 0);
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: ViewXdata Attivita: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> AddXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            try
            {
                ...
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<Attivita>('A', null, null, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: AddXdataModel Attivita: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> UpdateXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            ...
            {
                if (file == null || file.Length == 0) return BadRequest("File mancante");
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<Attivita>('M', icode, timestampHex, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: UpdateXdata Attivita: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> DeleteXdata(string icode, string timestampHex)
        {
            try
            {
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<Attivita>('D', icode, timestampHex, null, "", "", null, null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: DeleteXdata Attivita: " + ex.Message }); }
        }
        [BindProperty]
        ...
        public ActionResult Index(SelAttivita selobj)
        {
            ...
            //carica lista
            try { this.List = ErpContext.Instance.DogFactory.GetDog(dogId).List<Attivita>(this.Select, xrefTables, false, null, ref this._dogCache, null, -1); }  
