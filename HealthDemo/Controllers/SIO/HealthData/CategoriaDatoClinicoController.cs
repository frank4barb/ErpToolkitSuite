    ...
    public class CategoriaDatoClinicoController : ControllerErp
    {
        ...
        public JsonResult AutocompleteGetAll(string? modelPropertyName = null, [FromQuery] List<string> extraFieldNames = null)
        {
            try
            {
                ...
                return Json(ErpContext.Instance.DogFactory.GetDog(dogId).AutocompleteGetAll<CategoriaDatoClinico>(modelPropertyName: modelPropertyName, extraFieldNames: extraFieldNames));
            }
       ...
       // -- GET: lista tipi documento ----------------------------
        ...
        public IActionResult ViewXdata(string icode) 
        {
            try
            {
                ...
                BlobStreamResult blob = ErpContext.Instance.DogFactory.GetDog(dogId).OpenBlobStream<CategoriaDatoClinico>(icode, 0);
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: ViewXdata CategoriaDatoClinico: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> AddXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            try
            {
                ...
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<CategoriaDatoClinico>('A', null, null, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: AddXdataModel CategoriaDatoClinico: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> UpdateXdata(string icode, string timestampHex, string mref, string descr, string fmt, IFormFile file)
        {
            ...
            {
                if (file == null || file.Length == 0) return BadRequest("File mancante");
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<CategoriaDatoClinico>('M', icode, timestampHex, mref, descr, fmt, file.OpenReadStream(), null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: UpdateXdata CategoriaDatoClinico: " + ex.Message }); }
        }
        ...
        public async Task<IActionResult> DeleteXdata(string icode, string timestampHex)
        {
            try
            {
                ModelXdata xdataResult = await ErpContext.Instance.DogFactory.GetDog(dogId).MntXdataBlobStreamAsync<CategoriaDatoClinico>('D', icode, timestampHex, null, "", "", null, null);
                return Json(new
                {
            ...
            catch (Exception ex) { return Json(new { error = "Problemi in accesso al DB: DeleteXdata CategoriaDatoClinico: " + ex.Message }); }
        }
        [BindProperty]
        ...
        public ActionResult Index(SelCategoriaDatoClinico selobj)
        {
            ...
            //carica lista
            try { this.List = ErpContext.Instance.DogFactory.GetDog(dogId).List<CategoriaDatoClinico>(this.Select, xrefTables, false, null, ref this._dogCache, null, -1); }  
