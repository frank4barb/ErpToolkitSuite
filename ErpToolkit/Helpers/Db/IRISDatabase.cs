...
namespace ErpToolkit.Helpers.Db
{
    public class IRISDatabase : IDatabase, IDisposable
    {
        ...
        private IRISClientWrapper _IRISClientWrapper;

        private string _connectionString;
        private object _transaction = null;  //IRISTransaction
        private static readonly object _lock = new object();

        public IRISDatabase(string connectionString)
        {
            _IRISClientWrapper = new IRISClientWrapper();
        ...
        private object OpenConnection()  //IRISConnection
        {
            object connection = null;  //IRISConnection
            lock (_lock)
            {
                connection = _IRISClientWrapper.CreateIRISConnection(_connectionString);  //connection = new IRISConnection(_connectionString);
                _IRISClientWrapper.OpenIRISConnection(connection);  //connection.Open();
                return connection;
            }
        }
        private void CloseConnection(object connection)
        {
            lock (_lock)
            {
                if (connection != null && _IRISClientWrapper.GetStateIRISConnection(connection) == ConnectionState.Open)  //if (connection != null && connection.State == ConnectionState.Open)
                {
                    _IRISClientWrapper.CloseIRISConnection(connection);  //connection.Close();
                    _IRISClientWrapper.DisposeIRISConnection(connection);  //connection.Dispose();
                    connection = null;
                }
            }
        }
        ...
        public IDbConnection NewConnection()
        {
            ...
            if (_transaction != null) return _IRISClientWrapper.GetConnectionIRISTransaction(_transaction);  //if (_transaction != null) return _transaction.Connection;
            else return (IDbConnection)OpenConnection();
        }
        public void ReleaseConnection(IDbConnection connection)
        {
            //return (IDbConnection)new SqlConnection(_connectionString);
            if (_transaction == null) CloseConnection((object)connection);  //CloseConnection((IRISConnection)connection);
        }

        public IDbCommand NewCommand(string sql, IDbConnection connection)
        {
            return (IDbCommand)_IRISClientWrapper.CreateIRISCommand(sql, (object)connection, _transaction);  //return (IDbCommand)new IRISCommand(sql, (IRISConnection)connection, _transaction);
        }
        public DataTable QueryReader(IDbCommand command, int maxRecords)
        {
            using (IDisposable adapter = _IRISClientWrapper.CreateIRISDataAdapter((object)command))  //using (IRISDataAdapter adapter = new IRISDataAdapter((IRISCommand)command))
            {
                DataTable result = new DataTable();
                if (maxRecords < 0) _IRISClientWrapper.FillIRISDataAdapter(adapter, result);  //if (maxRecords < 0) adapter.Fill(result); 
                else _IRISClientWrapper.FillIRISDataAdapter(adapter, maxRecords, result); //else adapter.Fill(0, maxRecords, result); // restituisce maxRecords righe  
                return result;
            }
        }
        ...
        {
            object connection = OpenConnection();  //IRISConnection connection = OpenConnection(); 
            try
            {
                _transaction = _IRISClientWrapper.BeginTransactionIRISConnection(connection); //_transaction = connection.BeginTransaction();
        ...
        public void SavePointTransaction(string savePointName)
        {
            ...
            _IRISClientWrapper.SaveIRISTransaction(_transaction, savePointName);  // _transaction.Save(savePointName);
        }
        public void RollbackSavePoint(string savePointName)
        {
            ...
            _IRISClientWrapper.RollbackIRISTransaction(_transaction, savePointName); //_transaction.Rollback(savePointName);
        }
        ...
        public void CommitTransaction(string transactionName)
        {
            ...
            object connection = (object)_IRISClientWrapper.GetConnectionIRISTransaction(_transaction);  //IRISConnection connection = (IRISConnection)_transaction.Connection;
            try { _IRISClientWrapper.CommitIRISTransaction(_transaction); _IRISClientWrapper.DisposeIRISTransaction(_transaction); _transaction = null; }  //try { _transaction.Commit(); _transaction.Dispose(); _transaction = null; }
        ...
        public void RollbackTransaction(string transactionName)
        {
            ...
            object connection = (object)_IRISClientWrapper.GetConnectionIRISTransaction(_transaction);  //IRISConnection connection = (IRISConnection)_transaction.Connection;
            try { _IRISClientWrapper.RollbackIRISTransaction(_transaction); _IRISClientWrapper.DisposeIRISTransaction(_transaction); _transaction = null; }
            finally { CloseConnection(connection); }
        }
        ...
        {
            if (_IRISClientWrapper.IsIRISException(ex)) //if (ex is IRISException irisEx) 
            {
                int irisEx_ErrorCode = _IRISClientWrapper.GetErrorCodeIRISException(ex);    
                return irisEx_ErrorCode == -140 || irisEx_ErrorCode == -121; // Timeout or Deadlock
            }
            return false;
        }
        ...
        public bool HandleException(Exception ex)
        {
            if (_IRISClientWrapper.IsIRISException(ex))  //if (ex is IRISException irisEx)
            {
                ...
                switch (irisEx_ErrorCode) //switch (irisEx.Sqlcode)
                {
                    case -119:
